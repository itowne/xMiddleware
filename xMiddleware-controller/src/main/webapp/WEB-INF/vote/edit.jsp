...
<body class="nav-md">
                    ...
                    <form id="bizForm" data-parsley-validate class="form-horizontal form-label-left">
                      <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="text" name="voteTitle" class="form-control col-md-7 col-xs-12" value="${vote.voteTitle}">
                        </div>
                      </div>
                       <div class="form-group">
	...
	<script type="text/javascript">
		...
		function addItem(obj){
			var $itm=subVoteHtml(null);
			$(obj).parent().parent().after($itm);
		}
		function subVoteHtml(obj){
			var subVoteTitle="";
			var subVoteIdx="";
			var subVoteId = "";
			if(obj!=null){
				subVoteTitle=obj.voteName;
				subVoteIdx = obj.idx;
				subVoteId = obj.voteItemRelationId;
			}
			var $itm=$('<div class="form-group">'
				...
				+'投票项目 <input type="text" name="subVote" autocomplete="off" class="form-control" value="'+subVoteTitle+'"/>'
				+'顺序 <input type="text" name="subVoteIdx" autocomplete="off" class="form-control" value="'+subVoteIdx+'"/>'
				+'<input type="hidden" name="subVoteId" class="form-control" value="'+subVoteId+'"/>'
				+'</div>'
				+'</div>');
			return $itm;
		}
			...
			$.ajax({
				...
				success :function (data){
					if(data==undefined||data==""){
						xAlert("提示","修改投票失败","error");
						return;
					}
					if(data.resCode!=1){
						xAlert("提示","修改投票失败! "+data.resMsg,"error");
						return;
					}
					xAlert("提示","修改投票成功","success");
					setTimeout(function(){
						window.location.href="${ctx}/vote/index";
					},3000)
				} , 
				error :function (XMLHttpRequest, textStatus, errorThrown){
					xAlert("提示","修改投票失败! 请稍后再试","error");
				},
		...
		function initEditData(){
			...
			var voteSubObj = eval('(' + voteSub + ')');
			for(var i=0;i<voteSubObj.length;i++){
				var $itm=subVoteHtml(voteSubObj[i]);
				$(".ln_solid").before($itm);
			}
		}
